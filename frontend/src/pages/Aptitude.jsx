...
export default function Aptitude() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const ansArray = Object.entries(answers).map(([qId, selectedOption]) => ({
      ...
      toast.success(
        `Quiz completed! Score: ${res.data.data?.scorePercentage?.toFixed(1)}%`
      )
    } catch (e) {
      console.error(e)
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-green-700">
                Score:{' '}
                {parseFloat(submitted.scorePercentage || 0).toFixed(1)}% |
                {submitted.correctAnswers}/{submitted.totalQuestions} correct
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <p className="text-2xl font-bold text-green-600">
                {parseFloat(stats.overallAvgScore || 0).toFixed(1)}%
              </p>
            ...
            <div>
              <p className="text-2xl font-bold text-purple-600">
                {stats.categoryScores
                  ? Object.keys(stats.categoryScores).length
                  : 0}
              </p>
              <p className="text-xs text-gray-500">Categories Tried</p>
            </div>
          </div>
        </div>
      )}
              ...
              {attempts.slice(0, 10).map(a => (
                <tr key={a.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 font-medium">
                    {parseFloat(a.scorePercentage || 0).toFixed(1)}%
                  </td>
